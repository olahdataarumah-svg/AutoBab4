...
with col1:
    variabel_x = st.text_input("Variabel Independen (X)", placeholder="Contoh: Fasilitas Kantor")
    # Angka P-Value dipaksa pakai format titik (0.000) dan step 0.001
    p_value = st.number_input("Nilai P-Value / Sig. (Gunakan TITIK, Cth: 0.245)", format="%.3f", value=0.000, step=0.001)
with col2:
    variabel_y = st.text_input("Variabel Dependen (Y)", placeholder="Contoh: Motivasi Kerja")
    arah_hubungan = st.selectbox("Arah Hubungan", ["Positif", "Negatif"])

st.markdown("**Teori dan Penelitian Terdahulu:**")
# Menggunakan text_area agar bisa diisi banyak (di-Enter)
teori = st.text_area("Grand Theory / Ahli yang Digunakan", placeholder="Bisa isi lebih dari 1 (pisahkan dengan Enter).\nContoh:\n1. Hasibuan (2017)\n2. Mangkunegara (2019)")

jurnal_sejalan = st.text_area("Jurnal yang SEJALAN (Mendukung)", placeholder="Bisa isi lebih dari 1.\nContoh:\n1. Siregar (2021)\n2. Ahmad (2022)")
jurnal_berbeda = st.text_area("Jurnal yang BERBEDA (Opsional)", placeholder="Bisa isi lebih dari 1.\nContoh:\n1. Wibowo (2020)\nKosongkan jika tidak ada")
        ...
        with st.spinner("⏳ AI sedang menyusun paragraf pembahasan yang elegan..."):
            try:
                ...
                - Paragraf 2: Berikan alasan logis/praktis di lapangan MENGAPA hasil ini terjadi. 
                  * JIKA DITOLAK: Berikan alasan rasional dan akademis kenapa X tidak mempengaruhi Y.
                  * JIKA NEGATIF: Berikan alasan logis kenapa semakin tinggi X justru membuat Y menurun.
                  Kaitkan alasan praktis ini dengan grand theory dari ahli-ahli yang disebutkan.
                - Paragraf 3: Bandingkan dengan penelitian terdahulu. Nyatakan sejalan dengan jurnal-jurnal sejalan. Jika jurnal berbeda diisi, sebutkan bahwa hasil ini berbeda dengan penelitian tersebut dan berikan alasan perbedaan (misal beda sampel/karakteristik). Jika jurnal berbeda kosong, buat kalimat penutup yang elegan.
                
                Langsung berikan 3 paragraf tanpa kata pengantar atau penutup tambahan. Gunakan bahasa akademis formal yang rapi.
                """

                # TEMBAKAN KE MESIN OPENAI TERBARU (V1.0+)
